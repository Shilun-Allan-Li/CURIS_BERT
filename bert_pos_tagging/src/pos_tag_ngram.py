...
def joinParse(data_file):
    ...
    for sentence in sentences:
        l = len(sentence)
        for i in range(l):
            partialList = [(sentence[j]['form'], sentence[j]['upostag'], sentence.metadata) for j in range(l) if abs(i-j) <= args.ngram // 2]
            entireList.append([partialList, i if i < args.ngram // 2 else args.ngram // 2])
    return entireList
...
class PosDataset(data.Dataset):
    ...
    def __init__(self, tagged):
        sents, tags_li, original_sentences, idx = [], [], [], []
        for lis in tagged:
            sent, i = lis
        ...
        self.sents, self.tags_li, self.original_sentences, self.idx_li = sents, tags_li, original_sentences, idx

    def __len__(self):
        return len(self.sents)

    def __getitem__(self, idx):
        words, tags, sentences, index = self.sents[idx], self.tags_li[idx], self.original_sentences[idx], self.idx_li[idx]
        x, y = [], []
        is_heads = []
        l = len(words)
        for i in range(l):
        ...
        seqlen = len(y)
        return words[index], x, is_heads, tags[index], y, seqlen, sentences
...
def eval(model, iterator, case, seed):
    ...
    assert len(Words) == len(Is_heads) == len(Sentences)
    if args.large:
        save_file = "../out/result_ngram_large_{}_{}_{}".format(case, args.ngram, seed)
    else:
        save_file = "../out/result_ngram_base_{}_{}_{}".format(case, args.ngram, seed)
    with open(save_file, 'w') as fout:
        for words, is_heads, tags, y_hat, sentences in zip(Words, Is_heads, Tags, Y_hat, Sentences):
            y_hat = [hat for head, hat in zip(is_heads, y_hat) if head == 1]
            preds = [idx2tag[hat] for hat in y_hat]
            fout.write('{} {} {} {}\n'.format(words, tags, preds[0], sentences[0]['text']))
...
tagsTrain = list(set(word_pos[1] for sent in trainPos for word_pos in sent[0]))
