...
class Net(nn.Module):
    ...
    def forward(self, x, y, is_heads):
        ...
        for i in range(enc.size()[0]):
            ...
            k = 1
            last_word_idx = None
            last_word_vector = None
            for j in range(len(is_head)):
                if(is_head[j]==1):
                    if last_word_idx is not None:
                        enc[i,last_word_idx] = last_word_vector / k
                    last_word_vector = enc[i,j]
                    last_word_idx = j
                    k = 1
                else:
                    if last_word_idx is not None:
...
def eval(model, iterator, case, seed):
    ...
    if args.large:
        save_file = "../out/result_average_large_{}_{}".format(case, seed)
    else:
        save_file = "../out/result_average_base_{}_{}".format(case, seed)
